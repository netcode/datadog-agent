...
func TestChown(t *testing.T) {
	...
	t.Run("fchown", func(t *testing.T) {
		...
		if err != nil {
			...
			assert.Equal(t, event.Chown.File.UID, uint32(98), "wrong initial user")
			assert.Equal(t, event.Chown.File.GID, uint32(99), "wrong initial group")
	...
	t.Run("fchownat", func(t *testing.T) {
		...
		if err != nil {
			...
			assert.Equal(t, event.Chown.File.Inode, getInode(t, testFile), "wrong inode")
			assertRights(t, event.Chown.File.Mode, uint16(expectedMode), "wrong initial mode")
			assert.Equal(t, event.Chown.File.UID, uint32(100), "wrong initial user")
			assert.Equal(t, event.Chown.File.GID, uint32(200), "wrong initial group")
